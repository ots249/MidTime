...
export const LowStockBanner: React.FC<LowStockBannerProps> = ({
  ...
  return (
    <section aria-label="মজুত সতর্কতা" className="max-w-5xl mx-auto px-3 sm:px-6 pt-3">
      <div className="rounded-2xl border border-rose-200 dark:border-rose-900/70 bg-gradient-to-r from-rose-50/90 via-amber-50/90 to-orange-50/90 dark:from-rose-950/70 dark:via-amber-950/40 dark:to-orange-950/50 p-3.5 sm:p-4 shadow-xs transition-colors">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm sm:text-base font-bold text-rose-900 dark:text-rose-200 flex items-center gap-1.5">
        ...
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
          {alertedMedicines.map(({ med, calc }) => (
            <div
              key={med.id}
              className="flex items-center justify-between p-2.5 rounded-xl bg-white dark:bg-slate-900 border border-rose-100 dark:border-rose-900/50 shadow-2xs"
            >
              <div className="min-w-0 pr-2">
                <div className="flex items-center gap-1.5">
                  ...
                  />
                  <p className="text-xs font-bold text-slate-900 dark:text-white truncate">{med.name}</p>
                </div>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                  বাকি: <strong className="text-rose-600 dark:text-rose-400 font-semibold">{calc.formattedBangla}</strong>
                  {calc.daysRemaining < 999 && !calc.isOutOfStock && (
                    <span className="text-slate-400 dark:text-slate-500"> (আর {toBanglaNumber(calc.daysRemaining)} দিন)</span>
                  )}
                </p>
              </div>
              ...
              <button
                ...
                className="inline-flex items-center gap-1 text-[11px] font-medium text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/70 hover:bg-emerald-100 dark:hover:bg-emerald-900 border border-emerald-200 dark:border-emerald-800 px-2 py-1 rounded-lg transition-colors shrink-0 cursor-pointer"
