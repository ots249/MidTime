  ...
  return (
    <div className="max-w-5xl mx-auto px-3 sm:px-6 py-4 space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 sm:p-6 shadow-xs transition-colors">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-4 mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-teal-50 dark:bg-teal-950/80 hover:bg-teal-100 dark:hover:bg-teal-900 text-teal-800 dark:text-teal-300 text-xs font-semibold border border-teal-200 dark:border-teal-800 transition-colors cursor-pointer"
            >
        ...
        {selectedImage && (
          <div className="mt-4 p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 space-y-4">
            <div className="flex flex-col sm:flex-row gap-4 items-start">
              {/* Image thumbnail */}
              <div className="relative w-full sm:w-48 h-48 rounded-xl overflow-hidden bg-slate-900 border border-slate-300 dark:border-slate-700 shrink-0 group">
                <img
                  src={selectedImage}
                  alt="Prescription"
              ...
              <div className="flex-1 space-y-3">
                <div>
                  <h3 className="text-sm font-bold text-slate-800 dark:text-white">প্রেসক্রিপশনের ছবি প্রস্তুত!</h3>
                ...
                {scanError && (
                  <div className="p-3.5 rounded-xl bg-rose-50 dark:bg-rose-950/80 border border-rose-200 dark:border-rose-900 text-rose-800 dark:text-rose-300 text-xs space-y-2">
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 font-semibold text-xs cursor-pointer"
                      >
                ...
                <div className="flex flex-wrap items-center gap-2 pt-2">
                  ...
                  <button
                    ...
                    className="px-3 py-2 text-xs font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition-colors cursor-pointer"
                  >
                    বাতিল
                  </button>
                </div>
            ...
            {scanResult && (
              <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700 space-y-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span className="text-xs font-semibold text-teal-800 dark:text-teal-300 bg-teal-100 dark:bg-teal-950/80 border border-teal-200 dark:border-teal-800 px-2.5 py-1 rounded-full">
                    ...
                    return (
                      <div
                        ...
                        className={`p-3 rounded-xl border transition-all cursor-pointer flex items-start gap-3 ${
                          isChecked
                            ? "bg-white dark:bg-slate-850 border-teal-400 dark:border-teal-600 ring-1 ring-teal-200 dark:ring-teal-900/60 shadow-2xs"
                            : "bg-slate-100/70 dark:bg-slate-800/50 border-slate-200 dark:border-slate-800 opacity-60"
                        }`}
                      >
                        <input
                          type="checkbox"
                          ...
                          className="w-4 h-4 rounded text-teal-600 mt-0.5 cursor-pointer accent-teal-600"
                          onClick={(e) => e.stopPropagation()}
                        />

                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-1.5">
                            <strong className="text-sm font-bold text-slate-900 dark:text-white">{med.name}</strong>
                            {med.strength && (
                              <span className="text-[11px] px-1.5 py-0.2 rounded bg-slate-100 dark:bg-slate-800 font-semibold text-slate-700 dark:text-slate-300">
                                {med.strength}
                              </span>
                            )}
                            <span className="text-[11px] px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-400 font-medium">
        ...
        {prescriptions.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {prescriptions.map((pres) => (
              <div
                key={pres.id}
                className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xs hover:shadow-sm transition-all flex flex-col justify-between"
              >
                <div>
                  ...
                  <div className="p-4 space-y-2">
                    <h4 className="text-sm font-bold text-slate-900 dark:text-white line-clamp-1">{pres.title}</h4>
                    ...
                    {pres.extractedMedicines && pres.extractedMedicines.length > 0 && (
                      <div className="pt-2">
                        <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">যুক্ত করা ওষুধ:</span>
                        <div className="flex flex-wrap gap-1 mt-1">
                          {pres.extractedMedicines.map((m, mIdx) => (
                            <span
                              key={mIdx}
                              className="text-[10px] px-2 py-0.5 rounded-md bg-teal-50 dark:bg-teal-950/80 text-teal-800 dark:text-teal-300 border border-teal-200 dark:border-teal-800 font-medium"
                            >
                              {m.name}
                            </span>
                          ))}
                ...
                {/* Footer / Delete */}
                <div className="p-3 bg-slate-50 dark:bg-slate-800/60 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between transition-colors">
                  <button
                    type="button"
                    onClick={() => setPreviewPrescription(pres)}
      ...
      {previewPrescription && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-3xl max-h-[90vh] rounded-2xl overflow-hidden flex flex-col shadow-2xl">
            <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-white">
                  {previewPrescription.title}
                </h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">{previewPrescription.date}</p>
              </div>
              <button
                type="button"
